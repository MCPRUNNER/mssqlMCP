        ...
        try
        {
            ...
            try
            {
                ...
                if (await reader.ReadAsync(cancellationToken))
                {
                    ...
                    catalogInfo.CreatedDate = reader["created_time"] != DBNull.Value ? Convert.ToDateTime(reader["created_time"]) : DateTime.MinValue;
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                while (await reader.ReadAsync(cancellationToken))
                {
                    var folder = new SsisFolderInfo
                    {
                        ...
                        CreatedDate = reader["created_time"] != DBNull.Value ? Convert.ToDateTime(reader["created_time"]) : DateTime.MinValue
                    };
                ...
                try
                {
                    var projectsQuery = @"
                        SELECT 
                            ...
                            CASE 
                                WHEN EXISTS (
                                    SELECT 1 FROM [SSISDB].[catalog].[packages] pkg 
                    ...
                    while (await reader.ReadAsync(cancellationToken))
                    {
                        var project = new SsisProjectInfo
                        {
                            ...
                            CreatedDate = reader["created_time"] != DBNull.Value ? Convert.ToDateTime(reader["created_time"]) : DateTime.MinValue,
                            LastDeployedDate = reader["last_deployed_time"] != DBNull.Value ? Convert.ToDateTime(reader["last_deployed_time"]) : DateTime.MinValue,
                        ...
                        try
                        {
                            ...
                            using var packagesCmd = CreateCommandWithTimeout(packagesQuery, connection);
                            ...
                            while (await pkgReader.ReadAsync(cancellationToken))
                            {
                                var package = new SsisPackageInfo
                                {
                                    ...
                                    CreatedDate = pkgReader["created_time"] != DBNull.Value ? Convert.ToDateTime(pkgReader["created_time"]) : DateTime.MinValue,
                                    LastModifiedDate = pkgReader["last_validated_time"] != DBNull.Value ? Convert.ToDateTime(pkgReader["last_validated_time"]) : DateTime.MinValue
                                };
                        ...
                        {
                            _logger.LogWarning(ex, "Could not retrieve packages for project {ProjectName}", project.Name);
                            // Continue with what we have
                        }
                    }
                }
