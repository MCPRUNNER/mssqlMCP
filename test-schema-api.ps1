...
# Test all tables endpoint
Write-Host "`n-- Testing All Tables Endpoint --" -ForegroundColor Cyan
try {
...
Write-Host "`n-- Testing Schema Filtering --" -ForegroundColor Cyan
if ($schemas -and $schemas.Count -gt 0) {
    $testSchema = if ($schemas -contains "dbo") { "dbo" } else { $schemas[0] }
    
    Write-Host "Testing with schema: '$testSchema'..." -ForegroundColor White
    try {
        ...
        if ($wrongSchemas.Count -gt 0) {
            ...
            Write-Host "✓ Schema filter working correctly!" -ForegroundColor Green
        }
    }
    catch {
